...
const AddImageToProjectDialog = ({
  ...
  const noProject = projects.length <= 1;
  return(
    <Dialog
      open={showForm}
      onRequestClose={() => {
  ...
  >
    ...
    >
    { projects.filter( i => i.id !== project).map( project =>
      <MenuItem value={project.id} primaryText={project.name}/>
    )}
    </SelectField> : <Element typography="body2">You do not have another project to move to.</Element>
    }
  </Dialog>
)};
  ...
  bindActionCreators(
    {
      ...
      confirmMoveToProject,
      resetMoveToProject,
      goToProject: (newProject, asset) => push(`/projects/${newProject}/${asset}`)
    },
    dispatch
  );
const mapStateToProps = ({ projectList, moveToProject: { showForm, assetType, data } }) => ({
  showForm,
